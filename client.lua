...
function useFiretruckSiren(veh)
	...
	for k,v in ipairs(Config.voertuigen) do
		if vehname == GetHashKey(v.model) and v.sirene == 'brandweer' then
			return true	
		end
	end
	return false
end
function useAmbulanceSiren(veh)
	local vehname = GetEntityModel(veh)
	for k,v in ipairs(Config.voertuigen) do
		if vehname == GetHashKey(v.model) and v.sirene == 'ambulance' then
			return true	
		end
	end
		return false
...
function useNoSiren(veh)
	local vehname = GetEntityModel(veh)
	for k,v in ipairs(Config.voertuigen) do
		if vehname == GetHashKey(v.model) and v.sirene == 'geen' then
			return true	
		end
	end
		return false
		...
		if newstate ~= state_airmanu[veh] then
			...
			if newstate == 1 then
				...
				if useFiretruckSiren(veh) then
					...
					PlaySoundFromEntity(snd_airmanu[veh], "VEHICLES_HORNS_CAR_HORN_MED_1", veh, 0, 0, 0)
				end
...
local curplyped = nil
local curplyinveh = nil
local curplyveh = nil
Citizen.CreateThread(function()
	while true do
		curplyped = PlayerPedId()
		curplyinveh = IsPedInAnyVehicle(curplyped, false)
		curplyveh = GetVehiclePedIsUsing(curplyped)	
		Citizen.Wait(5000)
	end
end)
---------------------------------------------------------------------
Citizen.CreateThread(function()
	while true do
			...
			if curplyped ~= nil and curplyveh ~= nil and (curplyinveh ~= nil and curplyinveh) then
			
				----- IS DRIVER -----
				local veh = curplyveh
				if GetPedInVehicleSeat(veh, -1) == curplyped then
